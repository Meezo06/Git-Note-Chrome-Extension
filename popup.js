...
const showCurrentNotes = async () => {
	const currUrl = await getCurrentTabUrl()
	const isRepo = await isGithubRepo(currUrl);
	if(isRepo) {
		const repoName = getRepoName(currUrl);
...
addNoteBtn.addEventListener('click', async () => {
	...
	if (!urlInput.value) alert("Please Enter a URL"); 
		...
		let repoObj = await chrome.storage.local.get(repo);
		const isRepoStored = repo in repoObj;
		if (!isRepoStored) {
			repoObj = {
				[repo]:
				{
			...
			repoObj[repo].generalNotes.push(noteInput.value);
			await chrome.storage.local.set(repoObj);
		}
